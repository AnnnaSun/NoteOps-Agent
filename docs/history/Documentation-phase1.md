...
- Web proposal 展示层已对齐当前后端状态语义：
    - 后端生成 proposal 时返回 `PENDING_REVIEW`
    - 前端将 `PENDING_REVIEW` 视为可 `Apply` 的待处理状态，而不是只接受 `PENDING`
- proposal 状态样式也按该语义复用待处理展示，不再因为状态字面量不一致而失去操作入口
...
- 已做真实页面按钮级验收：
    ...
    - `生成 Proposal`
- 验收中确认的真实问题是：
    - proposal 在页面上成功生成，但状态为 `PENDING_REVIEW`
    - 修复前前端不会显示 `Apply`
- 本轮修复后，页面逻辑已按 `PENDING_REVIEW` 提供 `Apply` 入口
- 已重新跑通最小按钮回归：
    - `Generate Proposal -> Apply -> Rollback`
    - 页面状态依次确认为 `PENDING_REVIEW -> APPLIED -> ROLLED_BACK`

### 仍未覆盖

- 后端 proposal 状态契约仍以实际实现为准；如果未来统一改成其他状态字面量，需要同步前端状态映射
